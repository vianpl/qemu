...
static void hyperv_sync_shared_vtl_state(CPUState *active_cs, CPUState *next_cs)
{
    ...
    /* Only on VTL return */
    if (get_active_vtl(next_cs) < get_active_vtl(active_cs))
        restore_regs_from_vtl_control(active_cs, next_cs);

    cpu_synchronize_post_reset(next_cs);
}
...
int hyperv_hcall_vtl_call(CPUState *vtl0)
{
    ...
    set_vtl_entry_reason(vtl0, vtl1, HV_VTL_ENTRY_VTL_CALL);
    cpu_resume(vtl1);
    ...
    while (true) {
        ...
        if (events & POLLIN) {
            ...
            if (vtl0->stopped) {
                qemu_mutex_lock_iothread();
                hyperv_sync_shared_vtl_state(vtl0, vtl1);
                set_vtl_entry_reason(vtl0, vtl1, HV_VTL_ENTRY_INTERRUPT);
                qemu_mutex_unlock_iothread();
